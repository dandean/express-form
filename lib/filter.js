...
function Filter(fieldname) {
  ...
  this.run = function(formData) {
    var splitPos = fieldname.search(/\[/);
    stack.forEach(function(filter) {
      if (splitPos > 0) {
        var tableName = fieldname.slice(0, splitPos);
        var newFieldname = fieldname.slice(splitPos+1, fieldname.length - 1);
        formData[tableName][newFieldname] = filter(formData[tableName][newFieldname]);
      } else {
