  ...
  return function(req, res, next) {
    ...
    dataSources.forEach(function(source) {
      if (req[source] && !object.isString(req[source])) {
        ...
        if (passThrough) { // Let undeclared stuff come through.
          var stuff = JSON.parse(JSON.stringify(req[source])); // Shitty reference-killer.
          req.form = union(stuff, req.form);
        }
      }
    });
    ...
    routines.forEach(function(routine) {
      var result = routine.run(req.body, req.form, autoTrim);
