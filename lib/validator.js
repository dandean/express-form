...
Validator.prototype.length = Validator.prototype.len = function(min, max, message) {
  if (object.isUndefined(message) && object.isString(max)) {
    message = max;
    max = undefined;
  }
  ...
  return this.extend(function(value) {
    ...
    if (value.length < min) {
      throw new Error(message || "Value is too short");
    }
    if (!object.isUndefined(max) && value.length > max) {
      throw new Error(message || "Value is too long");
    }
  });
};
